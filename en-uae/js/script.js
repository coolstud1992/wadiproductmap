...
$('#next-device').on('click',function(){
	...
	$('#status').fadeIn()
	if(items.length>0){
		$.post('map.php',{item:wadi,souq:souq,items:items,cat:catname},function(data){console.log(data)})
	}
		id+=1
...
function checkMap(item){
	...
	$.ajax({type:'POST',url:'checkmap.php',data:{item:wadi},success: function(data){
		if(data=="error"){
				mappings="false"
				}
			else{
